...
class TorrentCreator:
    ...
    def _validate_paths(
        ...
        output_path = (
            output_path.resolve() if output_path else Path(f"{input_path.name}.torrent")
        )
    ...
    def _create_torrent(
        ...
        piece_size: int,
        flags: int = libtorrent.create_torrent.v1_only
        | libtorrent.create_torrent.modification_time,
    ...
    def _generate_piece_hashes(
        ...
        try:
            ...
            if "Operation canceled" in str(e):
                ...
                logger.warning(
                    "Piece hash generation failed, retrying with larger piece size"
                )
                new_piece_size = piece_size * 2
                if new_piece_size > self.config.max_piece_size:
                    raise CreationError(
                        "Failed to generate piece hashes even with maximum piece size"
                    )
                new_torrent = self._create_torrent(
                    torrent.files(), new_piece_size, flags
                )
                self._generate_piece_hashes(
                    new_torrent, temp_dir, new_piece_size, flags
                )
            else:
    ...
    def calculate_optimal_piece_size(self, total_size: int) -> int:
        ...
        if self.config.target_pieces:
            ...
            while num_pieces > max_pieces and piece_size < self.config.max_piece_size:
                piece_size *= 2
                num_pieces = math.ceil(total_size / piece_size)

            # Decrease piece size if we're below minimum pieces
            while num_pieces < min_pieces and piece_size > self.config.min_piece_size:
                piece_size //= 2
                num_pieces = math.ceil(total_size / piece_size)
    ...
    def create(
        ...
        try:
            ...
            with secure_temp_environment() as (temp_output, temp_dir):
                ...
                fs = libtorrent.file_storage()
                total_files, total_size = self._add_files(
                    fs, input_path, input_path.parent
                )

                if total_files == 0:
                    raise ValidationError(
                        f"No valid files found to add from {input_path}"
                    )
                ...
                if input_path.is_file():
                    ...
                    for file_path in input_path.rglob("*"):
                        if file_path.is_file() and PathSecurity.is_safe_path(
                            file_path, input_path.parent
                        ):
                            self._copy_with_sanitized_name(
                                file_path, temp_dir, input_path.parent
                            )
    ...
    def _add_files(
        ...
        if input_path.is_file():
            ...
            for root, dirs, files in os.walk(input_path):
                ...
                dirs[:] = [
                    d for d in dirs if not (d.startswith(".") or d.startswith(".."))
                ]
