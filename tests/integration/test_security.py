...
from torrent.exceptions import CreationError, ValidationError
...
def verify_torrent_contents(
    torrent_data: bytes, expected_files: list[tuple[str, int]]
) -> None:
    """
    ...
    if b"files" not in info:
        ...
        ), "Single file torrent should have exactly one file"
        name, size = expected_files[0]
        assert (
            info[b"name"].decode() == name
        ), f"Expected file name {name}, got {info[b'name'].decode()}"
        assert info[b"length"] == size, f"Expected size {size}, got {info[b'length']}"
        return

    # Handle multi-file case
    torrent_files = info[b"files"]
    assert len(torrent_files) == len(
        expected_files
...
def test_relative_symlink_inside_directory(temp_dir, creator):
    ...
    with open(result, "rb") as f:
        verify_torrent_contents(f.read(), [("content.txt", len("test content"))])
