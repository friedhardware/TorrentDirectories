...
def multilingual_media_dir(tmp_path: Path) -> Path:
    ...
    categories = {"Movies": movies, "Music": albums, "Books": books}
...
def test_large_file_handling(large_file_dir: Path, tmp_path: Path) -> None:
    ...
    cmd = [
        ...
        "-v",
        "--min-piece-size",
        "1M",  # Place before subcommand
        "--max-piece-size",
        "4M",  # Place before subcommand
        "file",
        str(large_file_dir),
...
def test_piece_size_handling(piece_size: str, sample_dir: Path, tmp_path: Path) -> None:
    ...
    cmd = [
        ...
        "-v",
        "--min-piece-size",
        piece_size,  # Place before subcommand
        "file",
        str(sample_dir),
        "http://tracker.example.com:6969/announce",
        "-o",
...
def test_multilingual_media_torrents(
    multilingual_media_dir: Path, tmp_path: Path
) -> None:
    ...
    for category in categories:
        ...
        assert (
            code == 0
        ), f"Failed to process {category} with stdout:\n{stdout}\nstderr:\n{stderr}"
        ...
        assert (
            len(torrent_files) == expected_count
        ), f"Expected {expected_count} torrent files for {category}, got {len(torrent_files)}"
        ...
        assert (
            processing_time < 30
        ), f"{category} processing took too long: {processing_time:.2f} seconds"
